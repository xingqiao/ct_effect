...
;(function() {
	...
	function _cube(params, callback) {
		...
		wrap.appendChild(d);
		d.style.cssText = 'margin:0;display:block;position:absolute;' + css['transform-style'] + ':preserve-3d;' + css['backface-visibility'] + ':hidden;' + css.transition + ':transform ' + duration + 'ms ease;' + css['backface-visibility'] + ':hidden;' + css.transform + ':perspective(' + perspective + 'px) translate3d(0,0,-' + dc + 'px)';
		c.style[css.transform] = 'translate3d(0,0,' + dc + 'px)';
		d.appendChild(c);
		if (params.target) {
			c = document.createElement('canvas');
			var ctx = c.getContext('2d');
			ctx.drawImage(params.img, 0, 0, c.width = w, c.height = h);
			c.style.cssText = 'position:absolute;top:0;left:0;' + css.transform + ':translate3d(' + (isLR ? (isBR ? '-100%' : '100%') : 0) + ',' + (isLR ? 0 : (isBR ? '-100%' : '100%')) + ',' + dc + 'px) rotate3d(' + rd + ', ' + (isTR ? '-' : '') + '90deg);' + css['transform-origin'] + ':' + (isLR ? (isTR ? '100%' : 0) + ' 50%' : '50% ' + (isTR ? 0 : '100%'));
			d.appendChild(c);
