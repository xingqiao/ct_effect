...
;(function() {
	...
	function _translate(params, callback) {
		...
		if (params.target) {
			...
			d.appendChild(c);
			c = d;
			var nextImg = document.createElement('canvas');
			var ctx = nextImg.getContext('2d');
			ctx.drawImage(params.img, 0, 0, nextImg.width = w, nextImg.height = h);
		...
		c.style.cssText = css.transition + ':transform ' + duration + 'ms ' + timingFunction;
		var is3d = params.mode != '2d', translate = 'translate3d(' + (isVert ? 0 : (isBR ? w : -w) + 'px') + ',' + (isVert ? (isBR ? h : -h) + 'px' : 0) + ',0)', rotate = '', rotate_time = 200;
		if (is3d) {
			// 旋转10度，产生3d效果
			setTimeout(function() {
	...
	window.addImgEffect && addImgEffect(['平移', 'translate'], function(callback) {
		...
		console.log(this.direct, this.direction)
